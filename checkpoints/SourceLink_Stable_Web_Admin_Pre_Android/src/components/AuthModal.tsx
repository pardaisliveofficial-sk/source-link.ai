...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleGitHubTokenLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If user is already logged in or connecting GitHub token
      const existingEmail = email.trim() || `${ghProfile.login}@users.noreply.github.com`;
      const user = await connectGitHubAccountApi(existingEmail, token, ghProfile.login).catch(() => {
        // Fallback for direct token sign in
        return {
          id: 'usr_' + Date.now(),
