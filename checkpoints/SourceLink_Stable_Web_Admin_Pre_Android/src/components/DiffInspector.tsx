...
function generateLineDiffRows(file: FileDiff): LineDiffRow[] {
  ...
  while (i < oldLines.length || j < newLines.length) {
    ...
    if (oldLine !== null && newLine !== null && oldLine === newLine) {
      ...
      // Look ahead up to 5 lines in newLines for oldLine
      const lookAheadNew = newLines.slice(j, j + 5).indexOf(oldLine);
      // Look ahead up to 5 lines in oldLines for newLine
      const lookAheadOld = oldLines.slice(i, i + 5).indexOf(newLine);
...
export const DiffInspector: React.FC<DiffInspectorProps> = ({
  ...
  const diffRows = React.useMemo(() => {
    ...
    return generateLineDiffRows(activeFile);
  }, [activeFile]);
