...
async function githubRequest(endpoint: string, token: string, options: RequestInit = {}) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 404) {
      throw new Error(
        `GitHub returned 404 Not Found. Please check:\n` +
        `1. Personal Access Token: Must have 'repo' scope (full control of private repositories).\n` +
        `2. Permissions: Ensure your GitHub account has write access to this repo.\n` +
        `3. Fine-grained PAT: Ensure 'Contents' permission is set to Read & Write.`
      );
    }
    if (response.status === 401) {
      throw new Error(`401 Unauthorized: GitHub Token is invalid or expired. Please re-connect GitHub.`);
    }
    if (response.status === 403) {
      throw new Error(`403 Forbidden: Token lacks write permissions or API rate limit was reached.`);
    }
...
export async function pushChangesToGitHub(
  ...
  // Push all files explicitly selected by the user
  const activeDiffs = diffsToPush;
  if (activeDiffs.length === 0) {
    throw new Error('No files selected to push. Please select at least one file.');
  }

  // 1. Get HEAD reference for branch (or fallback to repo default branch)
  ...
  try {
    ...
    try {
      ...
      if (defaultBranch !== branch) {
        const defaultRefData = await githubRequest(`/repos/${owner}/${repo}/git/ref/heads/${defaultBranch}`, token);
        headSha = defaultRefData.object.sha;
        const commitData = await githubRequest(`/repos/${owner}/${repo}/git/commits/${headSha}`, token);
        baseTreeSha = commitData.tree.sha;
      }
    } catch {
