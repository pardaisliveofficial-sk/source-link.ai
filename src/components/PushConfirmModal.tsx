      ...
      <div className="bg-white border border-gray-200 rounded-xl max-w-lg w-full p-6 text-gray-900 shadow-xl relative animate-in fade-in zoom-in-95 duration-150">
        ...
        {pushedResult ? (
          ...
          <div className="space-y-4">
            ...
            {/* Conflict Warning or Error Message */}
            {conflictWarning && (
              <div className={`p-3 border rounded-lg text-xs flex items-start gap-2 ${
                conflictWarning.startsWith('Push failed:')
                  ? 'bg-red-50 border-red-200 text-red-900'
                  : 'bg-amber-50 border-amber-200 text-amber-900'
              }`}>
                <AlertTriangle className={`w-4 h-4 shrink-0 mt-0.5 ${
                  conflictWarning.startsWith('Push failed:') ? 'text-red-600' : 'text-amber-600'
                }`} />
                <div className="min-w-0 flex-1">
                  <p className="font-semibold">
                    {conflictWarning.startsWith('Push failed:') ? 'Push Failed' : 'Remote Notice'}
                  </p>
                  <p className="text-[11px] mt-0.5 whitespace-pre-line leading-relaxed">
                    {conflictWarning}
                  </p>
                </div>
              </div>
