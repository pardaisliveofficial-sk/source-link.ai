...
export const DiffViewer: React.FC<DiffViewerProps> = ({
  ...
  const toggleSelectAll = () => {
    if (selectedPaths.size === totalChanged) {
      setSelectedPaths(new Set());
    } else {
      setSelectedPaths(new Set(diffs.filter(d => d.status !== 'unchanged').map(d => d.path)));
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div className="lg:col-span-5 bg-gray-50 border border-gray-200 rounded-lg p-3 flex flex-col space-y-2">
              ...
              <div className="flex items-center justify-between pb-2 border-b border-gray-200 text-xs text-gray-600">
                <label className="flex items-center gap-2 cursor-pointer font-semibold text-gray-800">
                  <input
                    type="checkbox"
                    checked={selectedPaths.size > 0 && selectedPaths.size === totalChanged}
                    onChange={toggleSelectAll}
                    className="accent-blue-600"
                  />
                  <span>Select All ({selectedPaths.size}/{totalChanged})</span>
                </label>
                <span className="text-[11px] font-mono text-gray-500">{filteredDiffs.length} files</span>
              </div>
